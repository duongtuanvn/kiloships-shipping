...
class Kiloships_API
{
     ...
     *
     * @param string $tracking_number Tracking number.
     * @param string $response_type   Optional. SUMMARY or DETAIL.
     * @return array|WP_Error Tracking data or error.
     */
    public function get_tracking($tracking_number, $response_type = 'SUMMARY')
    {
        if (empty($this->api_key)) {
        ...
        $url = $this->api_url . '/tracking/' . urlencode($tracking_number);
        if (in_array($response_type, array('SUMMARY', 'DETAIL'), true)) {
            $url .= '?responseType=' . $response_type;
        }
        ...
        if ($http_code >= 400) {
            $msg = isset($data['message']) ? $data['message'] : (isset($data['error']['message']) ? $data['error']['message'] : 'Tracking request failed.');
            return new WP_Error('api_error', $msg, $data);
        }
    ...
    /**
     * Create SCAN form for multiple tracking numbers.
     *
     * @param array $payload mailingDate, entryFacilityZIPCode, destinationEntryFacilityType, fromAddress, shipment.trackingNumbers.
     * @return array|WP_Error Form data with scanFormImage URL or error.
     */
    public function create_scan_form($payload)
    {
        if (empty($this->api_key)) {
            return new WP_Error('missing_api_key', 'Kiloships API Key is missing.');
        }
        ...
        if ($http_code >= 400) {
            $msg = isset($data['message']) ? $data['message'] : (isset($data['error']['message']) ? $data['error']['message'] : 'SCAN form request failed.');
            return new WP_Error('api_error', $msg, $data);
        }

        if (empty($data['scanFormImage'])) {
            return new WP_Error('api_error', 'Invalid response: missing scan form URL.', $data);
        }
